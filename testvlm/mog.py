...
class VisualGrounder:
    def __init__(
        ...
        self.yesvg_file_path = "../hm3rscandata/xuanran/ceshioutputscan/query_analysisto/today.csv"
    ...
    def eval_worker_image_only(self, vg_row1):
        ...
        for jj, jow in ddfile.iterrows():
            if jj == i:
                duiyingyesvgrow = jow
                break
        xinquery = duiyingyesvgrow["utterance"]
        vg_row["utterance"] = xinquery
        query = vg_row["utterance"]
    ...
    def eval(self):
        ...
        xiufu_FILE = "../hm3rscandata/xuanran/ceshioutputscan/query_analysisto/today.csv"
            ...
            for j, jow in xiufufile.iterrows():
                if j == i:
                    if (row["scan_id"] + "*" + jow["scan_id"] + "*" + str(row["target_id"]),
                        jow["utterance"]) not in completed_pairs:
                        pair = (i, row)
                        tasks.append(pair)
    ...
    def validate_final_results(self, results):
        ...
        for i, row in self.vg_file.iterrows():
            for j, jow in xiufufile.iterrows():
                if j == i:
                    vg_pairs.append(
                        (row["scan_id"] + "*" + jow["scan_id"] + "*" + str(row["target_id"]), jow["utterance"]))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--det_info_path",
        type=str,
        default="../hm3rscandata/xuanran/ceshioutputscan/image_instance_detectorto/*/chunk*/detection.pkl",
    )
    parser.add_argument(
        "--vg_file_path",
        type=str,
        default="../hm3rscandata/xuanran/ceshioutputscan/query_analysisto/*_relations_with_images_selected_diffconf_and_pkl_top250.csv",
    )
    parser.add_argument(
        "--matching_info_path",
        type=str,
        default="../hm3rscandata/scannet/ceshioutputscan/topatsyes/exhaustive_matching_ceshi.pkl",
    )
