...
from vlm_grounder1.grounder.prompts.visual_grounder_prompts import *
from vlm_grounder1.utils import *
from vlm_grounder1.utils import (
    DetInfoHandler,
    MatchingInfoHandler,
    OpenAIGPT,
...
class VisualGrounder:
    def __init__(
        ...
        self.yesvg_file_path = "../hm3rscandata/xuanran/ceshioutput360/query_analysisyes/yesterday360.csv"
        ...
        # self.openaigpt_config = {"model": openaigpt_type}
        ...
        # set VLM parameters
        self.gpt_max_retry = kwargs.get("gpt_max_retry", 2)
        assert (
        ...
        self.image_det_confidence = kwargs.get("image_det_confidence", 0.2)
        # 这是否则0.2的意思
    ...
    def get_gpt_select_bbox_index(
        ...
        while retry <= 3:
            ...
            try:
                ...
                if (gpt_response["content"][0] == '`'):
                    gpt_response["content"] = gpt_response["content"].strip()[8:-4]
    ...
    def get_gpt_pred_image_id(self, scene_id, query, messages, intermediate_output_dir):
        ...
        while retry <= 3:
            ...
            messages.append(gpt_message)
            if type(pred_image_id) == str:
                if pred_image_id[0] == "f":
                    pred_image_id = pred_image_id.split("-")[1]
    ...
    def get_image_id_bbox_vanilla(self, vg_row):
        ...
        for img_path in views_pre_selection_paths:
            print(img_path)
        ...
        while retry <= 3:
            ...
            if self.use_new_detections:
                ...
                print("det这里的image_path是啥，", image_path)
                print("看看这pred_target_class有什么问题：", pred_target_class)
    ...
    def eval_worker_image_only(self, vg_row1):
        ...
        if len(matched_image_ids) == 0:
            ...
            for key, value in self.scene_infos.infos[scene_id]["images_info"].items():
                matched_image_ids.append(key)
        print("给出的matched_id是啥形式,", matched_image_ids)
        # filter out those with invalid extrinsics, not possible that all match_image_ids are invalid
        matched_image_ids = [
            match_image_id
            for match_image_id in matched_image_ids
    ...
    def eval(self):
        ...
        xiufu_FILE = "../hm3rscandata/xuanran/ceshioutput360/query_analysisyes/yesterday360.csv"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", type=str, default="outputs/iccv_kaiyuan_v1"
    )
    parser.add_argument(
        "--scene_info_path",
        type=str,
        default="../hm3rscandata/xuanran/ceshioutput360/3rscan_instance_data/scenes_train_val_info_w_images.pkl",
    )  # * if change this, need to disable global cache of tools
    parser.add_argument(
        "--det_info_path",
        type=str,
        default="../hm3rscandata/xuanran/ceshioutput360/image_instance_detectorto/*/chunk*/detection.pkl",
    )
    parser.add_argument(
        "--vg_file_path",
        type=str,
        default="../hm3rscandata/xuanran/ceshioutput360/query_analysisto/*_relations_with_images_selected_diffconf_and_pkl_top250.csv",
    )
    parser.add_argument(
        "--matching_info_path",
        type=str,
        default="../hm3rscandata/scannet/ceshioutput360/topatsyes/exhaustive_matching_ceshi.pkl",
    )
    ...
    # openai setting
    parser.add_argument("--openaigpt_type", type=str, default="gemini-2.0-flash")
    parser.add_argument("--gpt_max_retry", type=int, default=2)
