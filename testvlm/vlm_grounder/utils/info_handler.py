...
class SceneInfoHandler:
    def __init__(self, info_path, posed_images_root="data/scannet/posed_images"):
        ...
        posed_images_root = "/mnt/afs/rsxu/humiao/vlmg1/vlm-grounder1/hm3rscandata/xuanran/ceshioutputscan/posed_images"
    ...
    def get_extrinsic_matrix(self, scene_id, image_id):
        image_id = self.convert_image_id_to_key(image_id, daicolor=False)
        return self.infos[scene_id]["images_info"][image_id][
            "extrinsic_matrix"
        ]  # N * 4 numpy array

    def get_image_path(self, scene_id, image_id):
        image_id = self.convert_image_id_to_key(image_id, daicolor=True)
        if image_id is None:
            return None
    ...
    def get_depth_image_path(self, scene_id, image_id):
        ...
        return os.path.join(self.posed_images_root, scene_id, f"{image_id}.depth.pgm")

    def convert_image_id_to_key(self, image_id, daicolor=False):
        if not daicolor:
            if isinstance(image_id, int):
                xin_id = f"frame-{image_id:06d}"
            else:
                if image_id.endswith("color"):
                    xin_id = image_id.split(".")[0]
                elif image_id[0]=="f":
                    xin_id = image_id
                elif image_id[0]=="0":
                    xin_id = f"frame-{int(image_id):06d}"
                else:
                    xin_id = image_id
        else:
            if isinstance(image_id, int):
                xin_id = f"frame-{image_id:06d}.color"
            else:
                if image_id.endswith("color"):
                    xin_id = image_id
                elif image_id[0]=="f":
                    xin_id = image_id + ".color"
                elif image_id[0]=="0":
                    xin_id = f"frame-{int(image_id):06d}.color"
                else:
                    xin_id = image_id
