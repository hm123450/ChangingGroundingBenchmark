...
sys.path.append("/mnt/afs/rsxu/humiao/vlmg1/vlm-grounder1/3rdparty/pats")
###加入这一步就是为了引入pats注意了。
import argparse
import os
import random
...
class Scannet(Dataset):
    def __init__(
            self,
            data_path="../../../ceshioutputscan/posed_images",
            scene_infos="../../../ceshioutputscan/3rscan_instance_data/scenes_train_val_info_w_images.pkl",
            scene_ids=None,
    ):
    ...
    def _get_all_pairs(self):
        ...
        for scene_id in self.scene_ids:
            ...
            all_images_files = [
                f"{scene_id}/frame-{i:06d}.color.jpg" for i in range(num_posed_images)
            ]
...
if __name__ == "__main__":
    ...
    """parser.add_argument(
        "--vg_file",
        type=str,
        default="../../grounding/3rscanceshiz/yesterday_5_updated2_relations.csv",
    )  # * no used"""
    ...
    model = PATS(args)
    model.config.checkpoint = os.path.join("/mnt/afs/rsxu/humiao/vlmg1/vlm-grounder1/3rdparty/pats/weights", model.config.checkpoint)
    model.config.checkpoint2 = os.path.join("/mnt/afs/rsxu/humiao/vlmg1/vlm-grounder1/3rdparty/pats/weights", model.config.checkpoint2)
    model.config.checkpoint3 = os.path.join("/mnt/afs/rsxu/humiao/vlmg1/vlm-grounder1/3rdparty/pats/weights", model.config.checkpoint3)
    model.load_state_dict()
    model = model.cuda().eval()

    #vg_file_path = args.vg_file
    with open('yesscan.txt', 'r') as file:
        lines = file.readlines()

    # 去除每行末尾的换行符，并存储到列表中
    uuid_list = [line.strip() for line in lines]
    scene_ids = uuid_list
    # sort
    scene_ids.sort()
    print("看看总长度，", len(scene_ids))
    scene_range_begin = args.scene_range_begin
    if args.scene_range_end == -1:
        scene_range_end = len(scene_ids)
    else:
        scene_range_end = args.scene_range_end

    output_dir = "../../../ceshioutputscan/match_result_ceshiyes"
    # output_dir = f"{output_dir}/{os.path.basename(vg_file_path).replace('.csv', '')}"
    print(f"Output dir is {output_dir}")
    mmengine.mkdir_or_exist(output_dir)

    scene_ids = scene_ids[scene_range_begin:scene_range_end]
    ...
    matching_results = {}
    print(f"Starting processing scene_ids from {scene_range_begin}:{scene_range_end}")
    index = 0
    for data in tqdm(scannet_loader):
        ...
        if scene_id not in matching_results:
    ...
    # save results
    mmengine.dump(matching_results, f"{output_dir}/scene_id_range_{scene_range_begin}_{scene_range_end}.pkl")
